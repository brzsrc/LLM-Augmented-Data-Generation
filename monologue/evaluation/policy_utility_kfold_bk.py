...
========================================================
Successor to policy_utility_baseline.py. Same MDP / DDQN / FQE machinery, but
re-architected to fix the two power problems we diagnosed:

  Problem 1 — only 7 test patients (single fixed split) -> bootstrap CIs useless.
  Problem 2 — FQE diverges on some fits (values blew up to 600+) -> heavy-tailed
              estimator noise on top of the sampling noise.

Fixes baked in here:
  * K-FOLD over patients (default 5): every one of the ~37 patients is held out
    in exactly one fold and evaluated by a DDQN/FQE NOT trained on them
    (cross-fitting). Per-patient values are POOLED across folds -> the bootstrap
    is now over all 37 patients, not 7.
  * FQE VALUE CLIPPING: the TD target is clamped to the analytic value range
    [r_min/(1-gamma), r_max/(1-gamma)] -> kills divergence.
  * MULTI-SEED MEDIAN: each fold trains FQE with several seeds and takes the
    per-patient median -> robust to the occasional bad fit, separates estimator
    noise from sampling noise.
  * DUAL-GPU: folds are distributed round-robin across the given devices
    (e.g. cuda:0,cuda:1) and run in parallel processes (spawn).

Because the bootstrap now resamples PRE-COMPUTED per-patient values (no FQE
retraining inside the bootstrap loop), it is essentially free, so B can be large.
Total FQE fits = n_folds * fqe_seeds (e.g. 25) vs ~151 in the old baseline.

Comparison statistics: paired bootstrap differences (P(A>B) + diff-CI), which —
unlike the old Wilcoxon-on-bootstrap — do not get spuriously significant as B
grows. (Wilcoxon is intentionally dropped; see paired_diff.csv.)

OUTPUTS (in --out_dir)
----------------------
- kfold_summary.csv          point estimate + bootstrap CIs per policy
- paired_diff.csv            P(A>B) + 95% diff-CI for every policy pair
- boot_values.pkl            per-patient value matrix + bootstrap matrix
- action_distributions.json  cross-fitted DDQN action distribution
- per_fold_ddqn/             trained DDQN weights per fold

USAGE (autoDL, two GPUs)
------------------------
    python policy_utility_kfold.py \
        --orig_csv ../data/cleaned_output.csv \
        --out_dir outputs/kfold \
        --n_folds 5 --devices cuda:0,cuda:1 --fqe_seeds 5
"""
import argparse
import json
import os
...
def get_args():
    ...
    p.add_argument("--orig_csv", required=True,
                   help="Real HeartSteps CSV (uid, datetime, send, jbsteps30, ...)")
...
# ============================================================================
# Data loading & MDP construction  (identical preprocessing to the baseline)
# ============================================================================
def load_and_preprocess(orig_csv):
    ...
    if not pd.api.types.is_numeric_dtype(orig["temperature"]):
        ...
        if unknown_temps:
            print(f"    WARNING: unknown temperature labels: {unknown_temps}")
        orig["temperature"] = orig["temperature"].map(TEMP_ORDER).astype(int)

    WEATHER_ORDER = {"Clear": 0, "MostlyCloudy": 1, "PartlyCloudy": 2,
                     "Overcast": 3, "Fog": 4, "Rain": 5, "Snow": 6}
